...
import { PORTALS, roleAllows } from '../../lib/portals.js';
import { sessionFrom, json } from '../../lib/http.js';

export default async function handler(req, res) {
  const s = await sessionFrom(req);
  if (!s) return json(res, 200, { ok: true, authenticated: false });
  const portals = PORTALS.filter(p => roleAllows(s.role, p)).map(p => ({ id: p.id, name: p.name, href: `/portal/${p.id}/home.html` }));
