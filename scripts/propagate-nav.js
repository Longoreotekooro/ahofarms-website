...
const isNewsPage = f => f.startsWith('news/');
...
function processPage(file) {
  ...
  if (!V6_PAGES.has(file) && !isPortalPage(file) && !isNewsPage(file)) {
  ...
  return V6_PAGES.has(file) || isPortalPage(file) || isNewsPage(file) ? 'v6' : 'legacy';
}
