...
// stay intact. They still get the sentinel nav (mechanically required - see
// report), but shifted above their own header via --nav-h rather than
// overlapping it.
...
function processPage(file) {
  ...
  if (navRe.test(html)) {
    ...
    navAction = 'stamped';
  } else if (NO_NAV_TAG_PAGES.has(file)) {
    ...
    expect(old, `${file}: no <nav> found and not in NO_NAV_TAG_PAGES`);
