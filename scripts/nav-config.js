...
// Staged launch (CEO, 2026-09-22): portal-flags.json decides which portals
// the public build exposes. Hidden portals vanish from the header, the
// drawer, the footer, the Connect sheet and the homepage; their production
// URLs are blocked by middleware.js. Flip a flag and re-run the propagate
// scripts to launch one.
const FLAGS = require('../portal-flags.json');
const isPublic = id => !!(FLAGS.portals[id] && FLAGS.portals[id].public);
const anyProfessionalPublic = ['prescriber', 'pharmacy', 'export-partner'].some(isPublic);
