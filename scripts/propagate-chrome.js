...
const NOINDEX_PAGES = new Set(['board.html', 'business.html', 'kaupapa.html',
...
function stampHtmlTag(html, file) {
  return html.replace(/<html\b([^>]*)>/i, (m, attrs) => {
    ...
    if (SOLID_NAV_PAGES.has(file) || isPortalPage(file) || file.startsWith('news/')) cls.push('aho-nav-solid');
    a = a.replace(/\s*class="[^"]*"/, '');
...
// Page metadata (audit, 2026-09-22): every page gets a canonical URL and,
// where the page does not already declare them, a description, Open Graph
// and Twitter card tags derived from its <title>. Stamped between the
// AHO:META sentinels so pages keep their own richer tags when they have them.
const META_S = ['<!-- AHO:META:START - generated by scripts/propagate-chrome.js, do not edit by hand -->', '<!-- AHO:META:END -->'];
function stampMeta(html, file) {
  const without = html.replace(new RegExp(esc(META_S[0]) + '[\\s\\S]*?' + esc(META_S[1]) + '\\n?'), '');
  const rawTitle = ((without.match(/<title>([^<]*)<\/title>/i) || [, SITE.name])[1]).trim().replace(/&amp;/g, '&');
  ...
  const url = SITE.url + cleanPathForFile(file);
  ...
  if (!/property="og:title"/i.test(without)) {
    lines.push(`<meta property="og:type" content="website">`, `<meta property="og:site_name" content="${escHtml(SITE.name)}">`,
      ...
      `<meta property="og:url" content="${url}">`, `<meta property="og:image" content="${SITE.url}/${SITE.defaultImage}">`,
  ...
  if (indexable) {
    ...
    if (file === 'index.html') {
      ...
      if (file.startsWith('portal/')) items.push({ name: 'Portals', item: SITE.url + '/portal' });
      if (file.startsWith('news/')) items.push({ name: 'News', item: SITE.url + '/news' });
  ...
  if (!anchor) return without;
  const idx = anchor.index + anchor[0].length;
  return without.slice(0, idx) + '\n' + block + without.slice(idx);
}
  ...
  <div class="aho-wrap">
    <div class="aho-footer-cols">
      <div class="aho-footer-brand">
        <a href="/" class="aho-footer-logo" aria-label="Aho Farms home"><span class="aho-mark" aria-hidden="true"></span></a>
