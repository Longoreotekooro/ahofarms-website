...
import { json, fail, readBody, requireMethod, sameOrigin, clientIp, limited, isEmail, newToken, isSecure } from '../../http.js';
...
export default async function handler(req, res) {
  ...
  if (await limited(`forgot:${clientIp(req)}`, 10, 900, { failClosed: true })) return fail(res, 429, 'Too many requests. Please try again later.');
  ...
  const link = `${origin}/portal/${portal.id}/reset?token=${token}`;
