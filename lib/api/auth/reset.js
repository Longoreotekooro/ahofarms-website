...
import { json, fail, readBody, requireMethod, sameOrigin, clientIp, limited } from '../../http.js';

export default async function handler(req, res) {
  if (!requireMethod(req, res, 'POST')) return;
  if (!sameOrigin(req)) return fail(res, 403, 'Cross-site request refused.');
  if (await limited(`reset:${clientIp(req)}`, 10, 900, { failClosed: true })) return fail(res, 429, 'Too many attempts. Please wait 15 minutes and try again.');
  ...
  return json(res, 200, { ok: true, message: 'Your password has been set. You can sign in now.', redirect: `/portal/${data.portal}/login` });
}
