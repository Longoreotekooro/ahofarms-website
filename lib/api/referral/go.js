...
export default async function handler(req, res) {
  ...
  const back = '/portal/consumers?notice=referral-unknown#get-connected';
  ...
  await updateLead(lead.id, { completedAt: lead.completedAt || now, lastHandoffAt: now, handoffCount: (lead.handoffCount || 0) + 1, status: ['matched', 'referred'].includes(lead.status) ? 'completed' : lead.status });
