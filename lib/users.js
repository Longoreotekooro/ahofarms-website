...
const kv = {
  ...
  async listPartners() { const vals = await redis('HVALS', 'partners'); return (vals || []).map(v => JSON.parse(v)); },
  async putPartner(p) { await redis('HSET', 'partners', p.id, JSON.stringify(p)); return p; },
  async deletePartner(id) { await redis('HDEL', 'partners', id); },
  async putLead(l) { await redis('SET', `lead:${l.id}`, JSON.stringify(l)); await redis('SET', `ref:${l.code}`, l.id); return l; },
  async addLead(l) { await this.putLead(l); await redis('LPUSH', 'leads', l.id); return l; },
  async getLead(id) { const j = await redis('GET', `lead:${id}`); return j ? JSON.parse(j) : null; },
  async getLeadByCode(code) { const id = await redis('GET', `ref:${code}`); return id ? this.getLead(id) : null; },
  async putSubscriber(s) { await redis('HSET', 'newsletter', s.email, JSON.stringify(s)); return s; },
  async listLeads() {
    const ids = await redis('LRANGE', 'leads', 0, 999);
    if (!ids || !ids.length) return [];
    const vals = await redis('MGET', ...ids.map(i => `lead:${i}`));
    return vals.filter(Boolean).map(v => JSON.parse(v));
  },
};
...
const memory = { tokens: new Map(), counters: new Map(), requests: [], partners: null, leads: [], subscribers: new Map() };
const envStore = {
  writable: false,
  ...
  // Partners and leads are held in memory per instance (lost on restart);
  // the Redis store is the durable option. PORTAL_PARTNERS (JSON array)
  // seeds the partner list.
  async listPartners() {
    if (!memory.partners) { try { memory.partners = JSON.parse(process.env.PORTAL_PARTNERS || '[]'); } catch { console.error('PORTAL_PARTNERS is not valid JSON'); memory.partners = []; } }
    return memory.partners;
  },
  async putPartner(p) { const l = await this.listPartners(); const i = l.findIndex(x => x.id === p.id); if (i >= 0) l[i] = p; else l.push(p); return p; },
  async deletePartner(id) { const l = await this.listPartners(); const i = l.findIndex(x => x.id === id); if (i >= 0) l.splice(i, 1); },
  async putLead(l) { const i = memory.leads.findIndex(x => x.id === l.id); if (i >= 0) memory.leads[i] = l; else memory.leads.unshift(l); return l; },
  async addLead(l) { return this.putLead(l); },
  async getLead(id) { return memory.leads.find(x => x.id === id) || null; },
  async getLeadByCode(code) { return memory.leads.find(x => x.code === code) || null; },
  async listLeads() { return memory.leads; },
  async putSubscriber(s) { memory.subscribers.set(s.email, s); return s; },
};
...
/* ---------- prescriber partners and consumer leads ---------- */
import { normalisePartner, pickPartner, newReferralCode } from './partners.js';
export async function listPartners() { return store().listPartners(); }
export async function savePartner(input) {
  const stored = await store().listPartners();
  const existing = input.id ? stored.find(p => p.id === input.id) : null;
  const p = normalisePartner(input, existing || {});
  if (!p.id) p.id = 'pp-' + randomUUID().slice(0, 8);
  p.updatedAt = new Date().toISOString();
  return store().putPartner(p);
}
export async function removePartner(id) { return store().deletePartner(id); }

// Stores the lead and assigns the partner for its market. Returns
// { lead, partner } (partner null when the market has none yet).
export async function createLead(fields, consent, meta = {}) {
  const partners = await store().listPartners();
  const partner = pickPartner(partners, fields.country, fields.region);
  const now = new Date().toISOString();
  const referralMethod = partner ? (partner.referralUrl ? 'qr-link' : 'email-intro') : 'manual';
  const lead = {
    id: randomUUID(), code: newReferralCode(), ...fields, consent, ageConfirmed: true,
    partnerId: partner ? partner.id : null, partnerName: partner ? partner.name : null, referralMethod,
    status: partner ? 'matched' : 'new', createdAt: now, matchedAt: partner ? now : null,
    referredAt: null, completedAt: null, handoffCount: 0, introEmailed: false, meta,
  };
...
export async function updateLead(id, patch) {
  ...
  return store().putLead(l);
}
export async function leadByCode(code) { return store().getLeadByCode(code); }
export async function saveSubscriber(email, role, meta = {}) {
  return store().putSubscriber({ email: norm(email), role, subscribedAt: new Date().toISOString(), meta });
}
export async function listLeads() { return store().listLeads(); }
