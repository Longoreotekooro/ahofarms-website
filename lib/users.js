...
export async function createLead(fields, consent, meta = {}) {
  ...
  const now = new Date().toISOString();
  const lead = {
    id: randomUUID(), code: newReferralCode(), ...fields, consent,
    partnerId: partner ? partner.id : null, partnerName: partner ? partner.name : null,
    status: partner ? 'referred' : 'new', createdAt: now, referredAt: partner ? now : null,
    handoffAt: null, handoffCount: 0, introEmailed: false, meta,
  };
