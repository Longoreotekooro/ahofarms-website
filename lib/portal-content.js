...
const clinicalCard = (portalId, r) => ({
  title: `${r.line.split(' · ')[0]} · ${r.name}`, kind: 'product', status: 'available', href: RANGE_PDF(portalId), action: 'Flower range sheet (PDF)',
  ...
  specs: [['THC / CBD', `${r.thc} / ${r.cbd}`], ['Time of day', `${r.time} · ${r.cls}`], ['Sedation', r.sedation], ['Clinical profile', r.profile],
          ['Prescribing matrix', r.matrix], ['Dominant terpenes (mg/g)', r.terpenes], ['Line', r.line]],
});
// Commercial view (pharmacies): format, pack, pricing and positioning.
const commercialCard = (portalId, r) => ({
  title: `${r.line.split(' · ')[0]} · ${r.name}`, kind: 'product', status: 'available', href: r.sheet ? `/portal/${portalId}/files/${r.sheet}` : RANGE_PDF(portalId), action: r.sheet ? 'Product sheet (PDF)' : 'Flower range sheet (PDF)',
  meta: `${r.genetics} · ${r.format} · ${r.unit}`, note: r.summary,
  specs: [['THC / CBD', `${r.thc} / ${r.cbd}`], ['Unit size', r.unit], ['Pricing', `${r.rrp} · ${r.perG}`], ['Positioning', `${r.time} · ${r.cls}`],
          ['Dominant terpenes (mg/g)', r.terpenes], ['Line', r.line]],
});
    ...
    sections: [
      { id: 'products', nav: 'Products', eyebrow: '01 · Products', title: 'Current Aho Farms medicinal cannabis products', lead: 'The range available to prescribe today, with detailed specifications for each product.',
        items: [
          ...
          live('RĀ · Oreo cultivar sheet', 'Two-page cultivar sheet: characteristics, patient use, preparation, storage, testing and batch traceability.', '/portal/prescriber/files/ra-oreo-product-sheet.pdf', { action: 'Open PDF' }),
