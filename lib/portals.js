...
// Markets Aho Farms serves (CEO, 2026-09-20). Every request form asks for
// one; add a market here and all three forms pick it up.
export const COUNTRIES = ['New Zealand', 'Germany', 'United Kingdom', 'Australia'];
...
export const CONSUMER = {
  ...
  tagline: 'Understand the access pathway and find an independent prescriber.',
...
export function portalHome(p) { return `/portal/${p.id}/home.html`; }
export function portalPath(p) { return `/portal/${p.id}/`; }
export function portalLogin(p) { return `/portal/${p.id}/login.html`; }
...
export const PUBLIC_PORTAL_PAGES = new Set(['login.html', 'request-access.html', 'forgot.html', 'reset.html']);
